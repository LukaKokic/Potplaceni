...
begin
	if (select count (*) from public.accommodation where realestateid = acc_REID) = 0 then
		insert into public.accommodation (realestateid, typeid, equippedid, latitude, longitude, address, townid, active)
		values (acc_REID, type_ID, equipped_ID, lat, long, addr, town_ID, ac);
